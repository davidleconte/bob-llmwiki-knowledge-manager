...
def test_generated_doc_content_withheld(tmp_kb):
    ...
    plant_document(
        tmp_kb, "concepts", "agent-output.md",
...
def test_include_unverified_returns_real_content(tmp_kb):
    ...
    for r in result.get("results", []):
        if "agent-output" in r.get("file", ""):
            content = r.get("content", "")
            assert KB_CONTENT_OPEN in content, (
                "Content must still be wrapped in delimiters"
            )
            assert body in content, (
                "Real content must be present when include_unverified=True"
            )
            return
