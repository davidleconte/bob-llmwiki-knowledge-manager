...
- Does not run the skill's own evals (the harness lives in `~/.bob/skills/skill-creator/`; the agent should invoke it separately if they want a 90% score).
- Does not analyze runtime/log behavior — that's `tools/skill_review_dashboard.py`.
- Does not review files outside `skills/` (the build system, `Makefile` and `service.yml` are out of scope; flag those for a human reviewer if changed).
