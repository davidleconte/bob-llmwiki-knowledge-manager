...
def generate_pdf(
    ...
    # Apply the Eisvogel template so titlepage/theme-color frontmatter actually
    # renders. Without this, titlepage-* variables are silently ignored.
    if eisvogel:
        cmd.extend(['--template', 'eisvogel', '--listings'])
    ...
    # Execute pandoc
    try:
        layout_type = "Mobile (6x9in)" if mobile else "Desktop (A4)"
...
def main():
    ...
    parser.add_argument('-e', '--eisvogel', action='store_true', help='Use Eisvogel template (required for titlepage/theme colors)')
