...
- **"Coverage"** = code coverage measured by `pytest --cov`, enforced by the `fail_under` gate in `pyproject.toml` (the single home for the number: **≥80%**). It is **not** the same as **test pass rate**. Do not conflate them. Point-in-time snapshot (from CI, 2026-07-14): 84.4% coverage; 683 passed / 23 skipped / 0 xfailed (the config→runtime xfails were wired in Phase 4; the +30 tests are the Phase-5 validation harness). Undated maturity claims elsewhere must defer to this file; a CI validator (`scripts/check_status_consistency.py`) enforces that the cited gate matches `pyproject.toml`.
